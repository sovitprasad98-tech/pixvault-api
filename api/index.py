...
# ─────────────────────────────────────────
#  CORS HEADERS — sabse zaroori fix
# ─────────────────────────────────────────
@app.after_request
def add_cors(response):
    response.headers["Access-Control-Allow-Origin"]  = "*"
    response.headers["Access-Control-Allow-Methods"] = "GET, POST, OPTIONS, PUT, DELETE"
    response.headers["Access-Control-Allow-Headers"] = "Content-Type, X-API-Key, X-Master-Key, Authorization"
    return response
...
def handle_options(path):
    ...
    resp.headers["Access-Control-Allow-Headers"] = "Content-Type, X-API-Key, X-Master-Key"
    return resp
...
def upload_to_firebase(file_bytes, filename, content_type):
    ...
    res = requests.post(upload_url, data=file_bytes,
    ...
    download_url = (
        f"https://firebasestorage.googleapis.com/v0/b/"
        f"{FIREBASE_STORAGE_BUCKET}/o/{encoded}"
...
def home():
    return jsonify({
        ...
        "base_url": BASE_URL,
        "endpoints": {
            "upload":       "POST /upload?key=PV-XXXX",
            "redirect":     "GET  /i/{code}",
            "generate_key": "POST /admin/generate-key",
            "list_keys":    "GET  /admin/keys",
            "toggle_key":   "POST /admin/toggle-key",
...
def upload_image():
    ...
    if "image" in request.files:
        ...
        return jsonify({"success": False, "error": "Send image file or {url:...}"}), 400

    if len(file_bytes) > 10 * 1024 * 1024:
        return jsonify({"success": False, "error": "Max 10MB"}), 413
    ...
    db_push(f"uploads/{safe_key}", {
        ...
        "original_url": long_url, "short_url": short_url,
        "size": len(file_bytes), "uploaded_at": datetime.utcnow().isoformat()
    })
...
def generate_key():
    ...
    db_set(f"api_keys/{safe_key}", {
        "key": new_key, "label": body.get("label", "Unnamed"),
        "active": True, "upload_count": 0,
        "created": datetime.utcnow().isoformat(), "last_used": None
    })
    return jsonify({"success": True, "api_key": new_key, "message": "API key generated!"}), 201
...
def list_keys():
    ...
    return jsonify({"success": True, "total": len(all_keys),
                    "keys": [v for v in all_keys.values()]})
...
def toggle_key():
    ...
    db_set(f"api_keys/{safe_key}/active", body.get("active", True))
...
def stats():
    ...
    uploads     = sorted(uploads_raw.values() if isinstance(uploads_raw, dict) else [],
                         key=lambda x: x.get("uploaded_at", ""), reverse=True)
    return jsonify({
        "success": True, "label": key_data.get("label"),
